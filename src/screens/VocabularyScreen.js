...
export default function VocabularyScreen({ navigation }) {
  ...
  const getDefinition = (item) => {
    return item[`definition_${i18n.language}`] || item["definition_fr"];
  };

  const getExample = (item) => {
    return item[`example_${i18n.language}`] || item["example_fr"];
  };
...
const styles = StyleSheet.create({
  ...
  catBtnActive: { backgroundColor: "#1a73e8" },
  ...
  expression: { fontSize: 20, fontWeight: "bold", color: "#1a73e8" },
  star: { fontSize: 24 },
  definition: { fontSize: 15, color: "#333", marginTop: 6 },
  example: { fontSize: 13, color: "#777", fontStyle: "italic", marginTop: 4 },
